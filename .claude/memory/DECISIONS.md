...
- **Status:** REVISITED — **superseded by ADR-017 (2026-07-03)**, which pulls all three Engines back into P1 graded scope. The context/rationale below is retained for history; the decision no longer holds.
...
### ADR-017 — ML Engines pulled back into P1 graded scope (supersedes ADR-013)
- **Status:** DECIDED
- **Context:** ADR-013 (2026-06-22) deferred all three ML Engines to P2 to de-risk the Phase-1 schedule, leaving Q-11 open pending supervisor sign-off. On 2026-07-03, cross-checking the codebase docs against the finalised proposal-defence deck surfaced a conflict: the deck presents "Trained ML Models" as **Objective 03** — a graded FYP2 deliverable — while the PRD/checklist had them as post-FYP P2. Author confirmed the deck's framing is the intended scope: the ML Engines are part of the graded build, and are the next development phase to code (P1 feature code is already complete).
- **Decision:** Reverse ADR-013. Move `FR-ENG-01..07` from P2 → **P1** in `arcana_prd.md` §12A (phase note + per-Engine headers) and §17.10, resolve Q-11, and add an ML-Engine criterion to the §24 release gate. In `checklist.md`, relocate the Engines from §2.1 (P2) into a new **§1.14** under Phase 1 and add a gate line. Graded core = the **Re-Rank Engine + shared `Engine` abstraction** (Must-priority FR-ENG-02/06, at least one trained Engine); Mastery + Document Classifier (Could-priority) land as schedule allows. Build order: `engines/base.py` ABC → `rerank.py` → `mastery.py`/`classifier.py`.
- **Why:** The docs must match the artefact the author will actually defend. Presenting a graded objective in the deck while the spec calls it post-FYP is the exact examiner-mismatch risk the doc-conflict protocol exists to prevent. The primary empirical claim is unaffected — Engines remain strictly additive (§23.4 arms), never displacing the §23.1 graph-vs-flat ablation (R-13), so the retrieval-quality evidence still stands alone even if only one Engine ships.
- **Rejected:** (a) Keeping Engines P2 and rewording the deck to call Objective 03 "future work" — rejected because the author wants the trained model as a graded contribution, not deferred. (b) Building all three to full training as a hard gate — rejected as schedule-risky; the gate requires *one* trained Engine (Re-Rank), the rest are best-effort.
- **Revisit if:** Supervisor sign-off sets a different bar (e.g. all three required, or Re-Rank alone insufficient) — update the §24 gate and §1.14 accordingly. The schedule risk ADR-013 flagged is now live again; if training/serving the Re-Rank Engine threatens the submission, the fallback is a pretrained-prior-only Engine with the domain-adaptation framed as future work (mirrors the Mastery Engine cold-start pattern).

### ADR-018 — Firestore via hand-rolled REST client, opt-in per store via `*_backend` flags
- **Status:** DECIDED
- **Context:** The front-half plan (2026-07-03 sequencing note in CONTEXT.md) required swapping `NotebookStore`/`EventStore`/`UserProfileStore` from JSONL/JSON-on-disk to Firestore for deployment. `google-cloud-firestore` pulls in `grpcio` + protobuf codegen; `pinecone_store.py` already set a REST-over-SDK precedent for this exact reason (DECISIONS.md pattern, no prior ADR needed since it predates this log).
- **Decision:** `api/stores/firestore_client.py` — a minimal async `FirestoreClient` over the Firestore REST v1 API (get/set/delete/list/listCollectionIds), authenticated via `google.oauth2.service_account.Credentials` (the one new dependency: `google-auth`, confirmed installable in this venv — unlike ADR-016's `reportlab`/`python-docx`). Each store (`FirestoreNotebookStore`, `FirestoreEventStore`, `FirestoreUserProfileStore`) is a second concrete class beside the existing JSONL/JSON one, selected per-store by a `Settings` flag (`notebook_backend`, `event_backend`, `profile_backend`, each `"jsonl"/"json" | "firestore"`, default the on-disk option). `UserProfileStore` was refactored from a single concrete class into an ABC with the get/upsert PATCH-merge logic as a template method, so both backends share identical merge semantics.
- **Why:** Per-store flags (not one global `persistence_backend` switch) let the author migrate incrementally and keep local dev on JSONL with zero Firestore credentials required — confirmed live: `uvicorn api.main:app` boots clean with defaults during this session. REST-over-SDK avoids the grpc/protobuf dependency weight and keeps the same respx-testable pattern as `pinecone_store.py`. One `FirestoreClient` instance is shared across all three stores that opt in (built once in `_build_shared_resources()`), not one per store.
- **Rejected:** `google-cloud-firestore` SDK — heavier dependency surface, harder to unit-test without a live emulator. A single `persistence_backend` flag for all three stores — rejected because it forces an all-or-nothing migration when the real rollout will likely move notebooks/events before profiles (smaller blast radius first).
- **Revisit if:** Firestore security rules or multi-region requirements need transactions/batched writes beyond single-document get/set — the REST client would need a `:commit` batch-write method added.

### ADR-019 — `@tool` decorators wired for all 19 agents (closes ADR-010)
- **Status:** DECIDED
- **Context:** ADR-010 deferred `@tool` decoration on WritingAgent/LearningAgent/SocraticAgent, noting the structural gap was open as of 2026-06-22. By 2026-07-03 the gap had silently widened to all 19 non-orchestrator/non-UI agents — `registry.tools_for()` returned an empty list for every agent, so Objective 02's "tool calling" story had no evidence a `@tool`-decorated catalog existed at all.
- **Decision:** Added `BaseAgent.run_as_tool(query, state=None)` — a thin adapter that calls `self.run()` against the live `AgentState` when supplied (preserving invariant #4, the shared ledger) or a fresh one otherwise. Every tier-2/3/4 agent (research, graph_agent, discovery, learning, socratic, writing, literature, contradiction, cross_doc, comparator, timeline, annotation, citation, visual_agent, document, fact_checker, memory, study_planner, web_search) gained exactly one `@tool(agent=..., tier=...)`-decorated method that delegates to `run_as_tool`. Verified end-to-end: `registry.tools_for("research")` now returns a real `ToolSpec` with a JSON-schema `required: ["query"]`, and `registry.select_tools([...], limit=12)` returns a deterministically-ordered, deduped, capped set.
- **Why:** One tool per agent (not one per internal method) matches the existing agent-as-specialist granularity — the LLM-driven router picks *which agent*, not which internal step of an agent to call, so a finer-grained tool surface would misrepresent the actual composability model (invariant #5: agents compose via `route_to_agent`, not via arbitrary tool graphs).
- **Rejected:** Decorating every internal helper method as its own tool — rejected as over-granular and risking `max_tools_per_prompt` (12) exhaustion for intents that route to multiple specialists.
- **Revisit if:** An agent needs to expose more than one distinct callable capability to the LLM router (e.g. CitationAgent's "format one citation" vs "export full bibliography" as genuinely separate tool calls) — add a second `@tool`-decorated method on that agent rather than reworking the pattern.

### ADR-020 — PlagiarismReport/AudioSummary (24/24 catalog close) use deterministic heuristics, not ML/TTS
- **Status:** DECIDED
- **Context:** The last 2 of 24 GenUI catalog components (`uiux_plan.md` §4) were `PlagiarismReport` (originality/AI-content flags) and `AudioSummary` (audio-overview player). Neither has a trained model or TTS service available in this project (both were P2-tagged specifically because of that dependency).
- **Decision:** `PlagiarismReport` is produced by `WritingAgent` on a keyword-gated path ("plagiar", "originality", "ai content", ...) using deterministic trigram overlap between the query passage and retrieved corpus chunks for `originalityScore`/similarity flags, plus a repeated-4-gram self-repetition ratio as an explicitly-labelled `aiLikelihood` heuristic (not a trained AI-content detector). `AudioSummary` is produced by `DocumentAgent` on a keyword-gated path ("audio overview", "podcast", ...): the LLM writes a spoken-style transcript + chapter labels, segment timings are estimated at 150 wpm, and `audioUrl` stays `null` (renderer shows a "generation pending" player state with the transcript as the primary content) since TTS synthesis is unbuilt.
- **Why:** Both catalog entries needed *a* producing agent to close FR-UI-02 (24/24) within this session's scope; building a real plagiarism-detection model or wiring a TTS provider would each be a separate P2-scale slice (new dependency, training data, or a paid API). The deterministic paths are honest about their limits — the renderer/copy explicitly say "heuristic" and "not a certified detector" (NFR-REL-01 honest degradation) rather than presenting either as authoritative.
- **Rejected:** Leaving both components schema-only with no producing agent (the pre-session state) — rejected because the task explicitly asked for 24/24, and a schema with zero producing agents is indistinguishable from "not built" per the walking-skeleton mandate (every component needs a real path to data before it counts as done).
- **Revisit if:** A real plagiarism-detection API or TTS provider becomes available (budget/dependency-wise) — swap the heuristic bodies for real calls behind the same `block_type` payload shape; the schema and renderer need no changes.
