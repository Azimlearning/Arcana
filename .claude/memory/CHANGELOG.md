...
### 2026-07-03 — ML Engines pulled back into P1 graded scope (docs only, no code yet)
- **Changed:** Docs-only session reversing the ML-Engine scope. `docs/arcana_prd.md`: §12A phase note rewritten (P2 → P1 graded), the three Engine sub-headers re-tagged `P2` → `P1` (Re-Rank marked "primary Engine deliverable"), §17.10 FR-ENG-01..07 Phase column P2 → P1 with a phase note, Q-11 marked resolved, and a new ML-Engine criterion added to the §24 release gate. `docs/checklist.md`: new **§1.14 ML Engines** under Phase 1 (ordered build list — `base.py` ABC → `rerank.py` → data/eval → `mastery.py`/`classifier.py` → fallback → QA pass), §1.13's "deferred to P2" note corrected to drop ML Engines, §2.1 collapsed to a pointer at §1.14, and a `≥ 1 ML Engine trained + integrated` line added to the P1 release gate. No `api/` code touched — `api/engines/` is still unbuilt (the `Engine` ABC does not exist yet).
- **Decided:** DECISIONS.md **ADR-017** (supersedes ADR-013, now marked REVISITED). Graded core = Re-Rank Engine + shared `Engine` abstraction (≥1 trained Engine); Mastery/Document Classifier are Could-priority, best-effort. Engines stay strictly additive (§23.4 arms) — the §23.1 graph-vs-flat ablation remains the standalone primary evidence (R-13).
- **Deviations:** Direct reversal of the 2026-06-22 ADR-013 deferral. Triggered by a doc-vs-artefact conflict: the finalised proposal-defence deck presents "Trained ML Models" as graded Objective 03, which the P2 deferral contradicted. Author chose to align the spec to the deck (build in FYP2) rather than reword the deck.
- **Known issues / next steps:** **Next code phase = build `api/engines/`** per checklist §1.14, starting with `base.py` (the `Engine` ABC) then the Re-Rank Engine — use the ordered list. Supervisor sign-off still open on whether one trained Engine is a sufficient graded bar (Q-11 residual). Schedule risk that ADR-013 flagged is live again — fallback is a pretrained-prior-only Re-Rank Engine if training/serving threatens the submission. `docs/handoff/CONTEXT.md` pitch + deferred sections updated to match.
