"""TimelineAgent - Tier 2. Chronological CitedSummary synthesis.

Specialises the CitedSummary path for queries about the history or temporal
development of a topic. Uses a timeline-structured prompt so the LLM organises
its response chronologically.
"""
...
class TimelineAgent(BaseAgent):
    """Tier-2 agent. Returns a CitedSummary shaped as a chronological narrative."""
    ...
    async def run(self, query: str, state: AgentState) -> AgentResult:
        ...
        if not chunks:
            return AgentResult(
                ...
                payload=_empty_payload("No documents retrieved for timeline."),
                status="partial",
        ...
        try:
            ...
            logger.exception("timeline.llm_failed")
            return AgentResult(
                agent_name=self.name,
