...
> Canonical repository layout for Arcana. Companion to `arcana_prd.md` (the full spec), `checklist.md` (the phased build plan), and `uiux_plan.md` (the design authority for everything under `web/`). Referenced from `CLAUDE.md`.
>
...
├── docs/                # arcana_prd.md, project_file_structure.md, checklist.md, uiux_plan.md, env_generation_guide.md, concept doc
...
### Frontend notes
- **`uiux_plan.md` is the design authority for everything in `web/`** — exact design tokens, the full 24-component catalog (with the panel + phase for each), the five modes and their panel proportions, the four mandatory interface states, and the key flows. Read it before building any shell or catalog component.
...
*Companion to `arcana_prd.md`, `checklist.md`, and `uiux_plan.md`. Keep this file in sync with the PRD: if a module moves or a boundary changes, update both.*
