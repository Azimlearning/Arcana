...
- **Companion files:** `project_file_structure.md` (the full repo tree, every file explained), `checklist.md` (the phased build checklist derived from this PRD), `uiux_plan.md` (the design authority — consolidates §13–§15 with exact tokens, the full component catalog, modes, states, and flows), and `env_generation_guide.md` (credentials & config). `CLAUDE.md` is the master index that ties all of these together.
